...
class HotkeyManager:
    ...
    def _listen(self):
        ...
        for number in range(1, 10):
            user32.RegisterHotKey(None, number, self.MOD_ALT, ord(str(number)))
        # Ctrl+Alt+Space 显示/隐藏窗口
        user32.RegisterHotKey(None, 100, self.MOD_CONTROL | self.MOD_ALT, self.VK_SPACE)
...
class DutyAssistant(tk.Tk):
    def __init__(self):
        ...
        self.title(APP_NAME)
        self.geometry(self.config_data.get("window_geometry", "390x620"))
        ...
        self.hotkeys = HotkeyManager(lambda key: self.after(0, self.handle_hotkey, key))
    ...
    def _build_ui(self):
        ...
        top.pack(fill="x")
        ttk.Label(top, text=APP_NAME, font=("Microsoft YaHei UI", 13, "bold")).pack(
            side="left"
        )
        ...
        self.status = tk.StringVar(value="Alt+1～9 快速粘贴；Ctrl+Alt+Space 显示/隐藏")
    ...
    def _build_reply_tab(self):
        ...
        self.reply_list = tk.Listbox(
            list_frame, activestyle="none", font=("Microsoft YaHei UI", 10)
        )
        ...
        self.reply_preview = tk.Text(self.reply_tab, height=6, wrap="word")
        self.reply_preview.pack(fill="x", pady=(3, 7))

        buttons = ttk.Frame(self.reply_tab)
        buttons.pack(fill="x")
        ttk.Button(buttons, text="复制并粘贴", command=self.use_selected_reply).pack(
            side="left"
        )
    ...
    def _build_polish_tab(self):
        ...
        self.raw_text = tk.Text(self.polish_tab, height=9, wrap="word")
        self.raw_text.pack(fill="both", expand=True, pady=(3, 8))
        ttk.Button(self.polish_tab, text="进行礼貌润色", command=self.polish).pack(
            anchor="e"
        )
        ttk.Label(self.polish_tab, text="润色结果").pack(anchor="w", pady=(8, 0))
        self.polished_text = tk.Text(self.polish_tab, height=9, wrap="word")
    ...
    def _build_docs_tab(self):
        ...
        self.doc_list = tk.Listbox(
            self.docs_tab, height=8, activestyle="none", font=("Microsoft YaHei UI", 10)
        )
        ...
        self.doc_preview = tk.Text(self.docs_tab, wrap="word")
    ...
    def refresh_replies(self):
        ...
        for row in self.reply_rows:
            prefix = f"Alt+{row['shortcut']}  " if row["shortcut"] else ""
    ...
    def handle_hotkey(self, key: int):
        ...
        if row:
            self.repo.mark_used(row["id"])
            self.copy_and_maybe_paste(row["content"], True)
